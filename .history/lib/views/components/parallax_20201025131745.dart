...
class _ParallaxState extends State<Parallax> with SingleTickerProviderStateMixin {
  ...
  var xAccStore = ListQueue<double>()..addAll(List.generate(_zAmount, (index) => 0.0));
...
//-------------------------------------s
  void slide() {
    ...
    double zOffset =
        (xAccStore.reduce((first, next) => first + next) / _zAmount / 10).clamp(-10.0, 10.0);
    slideOffset = Offset(zOffset + slideAmount, yOffset * slideAmount);
  }
