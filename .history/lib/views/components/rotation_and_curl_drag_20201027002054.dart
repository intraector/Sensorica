...
class _ViewRotationDragState extends State<ViewRotationDrag> with TickerProviderStateMixin {
  ...
  Future<List<Uint8List>> loadImages() async {
    ...
    for (int index = 1; index <= 182; index++) {
      futures.add(
          rootBundle.load('assets/brushFullHD/output${index.toString().padLeft(3, '0')}.png')
    ...
    images = uint8List.map<Image>((e) => Image.memory(e, gaplessPlayback: true, cacheHeight: ,).toList();
    return uint8List;
  }
}
